...
# full_set.select_dtypes(include=numerics).count()
...
dropped_df = df_to_be_prepared[drop_cols]
for c in dropped_df.columns:
    print(dropped_df[c].value_counts())
...
full_set.hist(figsize=(40,40))


full_set.special_conditions_at_site.value_counts()/full_set.special_conditions_at_site.count()
...
df_age_bands.age_band_of_casualty.value_counts()

# over 66
df_age_bands["age_band_of_casualty"] = np.where(df_age_bands["age_band_of_casualty"] >= 10 , 10, df_age_bands["age_band_of_casualty"])

df_age_bands['age_band_of_casualty'] = df_age_bands['age_band_of_casualty'].rank(method="dense").astype("int32")

# df_age_bands["age_band_of_casualty"] = np.where(df_age_bands["age_band_of_casualty"] == 1 , 1,  df_age_bands["age_band_of_casualty"] -2)
...
df_cas_type[plot_var].value_counts()
...
df_ageband['age_band_of_driver'] = df_ageband['age_band_of_driver'].rank(method="dense").astype("int32")
